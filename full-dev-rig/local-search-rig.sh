...
def gen_bat():
    ...
    ap('REM  It also asks which browser rendering engine Firecrawl should use -')
    ap('REM  Playwright (default) or Browserless (stealth mode, better block')
    ap('REM  avoidance) - and writes COMPOSE_PROFILES so only that one starts.')
    ...
    ap('')
    # Quick setup gate — skip the six numbered steps entirely
    ap('echo ============================================================')
    ap('echo   Quick setup')
    ap('echo ============================================================')
    ap('echo   Defaults: %DEFAULT_TARGET%, SearXNG 9990, Firecrawl 9991,')
    ap('echo   Playwright engine, no local LLM, no Firecrawl account.')
    ap('echo   You can change any of this later by editing .env and running')
    ap('echo   Update.bat.')
    ap('set "USE_DEFAULTS="')
    ap('set /p USE_DEFAULTS="  Use default settings? [Y/n]: "')
    ap('if /i "!USE_DEFAULTS!"=="n" goto full_setup')
    ap('')
    ap('set "TARGET=%DEFAULT_TARGET%"')
    ap('for %%I in ("!TARGET!") do set "TARGET=%%~fI"')
    ap('set "SEARXNG_PORT=9990"')
    ap('set "FIRECRAWL_PORT=9991"')
    ap('set "BROWSER_ENGINE=playwright"')
    ap('set "PW_URL=http://playwright-service:3000/scrape"')
    ap('set "OPENAI_BASE_URL="')
    ap('set "OPENAI_API_KEY="')
    ap('set "MODEL_NAME="')
    ap('set "FC_API_KEY="')
    ap('set "FC_API_URL="')
    ap('echo   Using defaults - see summary below.')
    ap('echo.')
    ap('goto setup_done')
    ap('')
    ap(':full_setup')
    ap('echo --- Step 1 of 6: Install location --------------------------')
    ...
    ap('')
    ap(':ask_searxng')
    ap('echo --- Step 2 of 6: SearXNG port (default 9990) --------------')
    ap('set "SEARXNG_PORT="')
    ...
    ap('')
    ap(':ask_firecrawl')
    ap('echo --- Step 3 of 6: Firecrawl port (default 9991) ------------')
    ...
    ap('')
    ap('echo.')
    ap('echo --- Step 4 of 6: Browser rendering engine (default: Playwright) ---')
    ap('echo   Firecrawl needs a headless-browser service for JS-rendered pages.')
    ap('echo   Playwright  - the classic Firecrawl engine ^(default^).')
    ap('echo   Browserless - stealth mode ^(masks automation fingerprints^);')
    ap('echo                 better at avoiding Cloudflare/bot-check blocks.')
    ap('set "USE_BROWSERLESS="')
    ap('set /p USE_BROWSERLESS="  Use Browserless instead of Playwright? [y/N]: "')
    ap('if /i "!USE_BROWSERLESS!"=="y" (')
    ap('  set "BROWSER_ENGINE=browserless"')
    ap('  set "PW_URL=http://browserless:3000/scrape"')
    ap(') else (')
    ap('  set "BROWSER_ENGINE=playwright"')
    ap('  set "PW_URL=http://playwright-service:3000/scrape"')
    ap(')')
    ap('echo     ^(Engine: !BROWSER_ENGINE!^)')
    ap('')
    ap('echo.')
    ap('echo --- Step 5 of 6: Local LLM (optional) ---------------------')
    ...
    ap(')')
    ap('')
    # Step 6: optional Firecrawl account (unlocks the account-gated tools)
    ...
    ap('echo --- Step 6 of 6: Firecrawl account (optional) -------------')
    ...
    ap('echo   Browser engine: !BROWSER_ENGINE!')
    ...
    ap('>> "!TARGET!\\.env" echo.')
    ap('>> "!TARGET!\\.env" echo # ---- Browser rendering engine for Firecrawl ^(installer Step 4^) ----')
    ap('>> "!TARGET!\\.env" echo COMPOSE_PROFILES=!BROWSER_ENGINE!')
    ap('>> "!TARGET!\\.env" echo PLAYWRIGHT_MICROSERVICE_URL=!PW_URL!')
    ap('>> "!TARGET!\\.env" echo.')
    ap('>> "!TARGET!\\.env" echo # ---- Browserless token ^(only used if COMPOSE_PROFILES=browserless above^) ----')
...
def gen_sh():
    ...
    ap('#  It also asks which browser rendering engine Firecrawl should use -')
    ap('#  Playwright (default) or Browserless (stealth mode, better block')
    ap('#  avoidance) - and writes COMPOSE_PROFILES so only that one starts.')
    ...
    ap('')
    ap('quick_setup() {')
    ap('  TARGET="$DEFAULT_TARGET"')
    ap('  mkdir -p "$TARGET"')
    ap('  TARGET="$(cd "$TARGET" && pwd)"')
    ap('  SEARXNG_PORT=9990')
    ap('  FIRECRAWL_PORT=9991')
    ap('  BROWSER_ENGINE="playwright"')
    ap('  PW_URL="http://playwright-service:3000/scrape"')
    ap('  OPENAI_BASE_URL=""; OPENAI_API_KEY=""; MODEL_NAME=""')
    ap('  FC_API_KEY=""; FC_API_URL=""')
    ap('  say "  Using defaults - see summary below."')
    ap('}')
    ap('')
    ap('full_setup() {')
    ap('hdr "Step 1 of 6: Install location"')
    ...
    ap('}')
    ap('')
    ap('hdr "Step 2 of 6: SearXNG port (default 9990)"')
    ap('while true; do')
    ...
    ap('done')
    ap('')
    ap('hdr "Step 3 of 6: Firecrawl port (default 9991)"')
    ap('while true; do')
    ...
    ap('done')
    ap('')
    ap('hdr "Step 4 of 6: Browser rendering engine (default: Playwright)"')
    ap('say "  Firecrawl needs a headless-browser service for JS-rendered pages."')
    ap('say "  Playwright  - the classic Firecrawl engine (default)."')
    ap('say "  Browserless - stealth mode (masks automation fingerprints);"')
    ap('say "                better at avoiding Cloudflare/bot-check blocks."')
    ap('printf "  Use Browserless instead of Playwright? [y/N]: "')
    ap('read -r USE_BROWSERLESS')
    ap('if [ "$(lower "$USE_BROWSERLESS")" = "y" ]; then')
    ap('  BROWSER_ENGINE="browserless"')
    ap('  PW_URL="http://browserless:3000/scrape"')
    ap('else')
    ap('  BROWSER_ENGINE="playwright"')
    ap('  PW_URL="http://playwright-service:3000/scrape"')
    ap('fi')
    ap('say "    (Engine: $BROWSER_ENGINE)"')
    ap('')
    ap('hdr "Step 5 of 6: Local LLM (optional)"')
    ...
    ap('')
    # Step 6: optional Firecrawl account (unlocks the account-gated tools)
    ap('hdr "Step 6 of 6: Firecrawl account (optional)"')
    ...
    # Quick setup gate — skip the six numbered steps entirely
    ap('say "${BOLD}============================================================${RESET}"')
    ap('say "${BOLD}  Quick setup${RESET}"')
    ap('say "${BOLD}============================================================${RESET}"')
    ap('say "  Defaults: $DEFAULT_TARGET, SearXNG 9990, Firecrawl 9991,"')
    ap('say "  Playwright engine, no local LLM, no Firecrawl account."')
    ap('say "  You can change any of this later by editing .env and running"')
    ap('say "  ./update.sh."')
    ap('printf "  Use default settings? [Y/n]: "')
    ap('read -r USE_DEFAULTS || USE_DEFAULTS=""')
    ap('if [ "$(lower "$USE_DEFAULTS")" = "n" ]; then full_setup; else quick_setup; fi')
    ap('')
    # Summary + confirm
    ap('echo')
    ap('say "${BOLD}============================================================${RESET}"')
    ...
    ap('say "  Browser engine: $BROWSER_ENGINE"')
    ...
    ap('  echo')
    ap('  echo "# ---- Browser rendering engine for Firecrawl (installer Step 4) ----"')
    ap('  echo "COMPOSE_PROFILES=$BROWSER_ENGINE"')
    ap('  echo "PLAYWRIGHT_MICROSERVICE_URL=$PW_URL"')
    ap('  echo')
    ap('  echo "# ---- Browserless token (only used if COMPOSE_PROFILES=browserless above) ----"')
...
#   Gate: use defaults? -> n (full setup, to exercise custom values),
#   Step 1: target folder, Step 2: searxng port, Step 3: firecrawl port,
#   Step 4: browserless? -> n (playwright default), Step 5: connect LLM? -> n,
#   Step 6: Firecrawl account? -> n,  confirm -> y
printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
  "n" \
  "$TGT_DIR" \
  "" \
...
# --- quick-setup test: accepting the new gate's default (Y) must skip -------
#     all six numbered steps and install with the documented defaults ---------
echo
echo "===== quick-setup install: 'Use default settings? [Y/n]' -> Enter ====="
QUICKHOME="$TESTROOT/quickhome"
mkdir -p "$QUICKHOME"
printf '\n' | env HOME="$QUICKHOME" PATH="$MOCKBIN:$PATH" \
  "$SRC_DIR/install-local-search.sh" > "$TESTROOT/quick.log" 2>&1
QRC=$?
echo "Installer exit code: $QRC"
tail -15 "$TESTROOT/quick.log"
echo "--------------------------------"
QTGT="$QUICKHOME/local-search"
if [ "$QRC" = 0 ] && [ -d "$QTGT" ]; then
  echo "[OK] quick-setup installed to the default folder ($QTGT)"
else
  echo "[FAIL] quick-setup did not install to the default folder"; PASS=0
fi
if grep -q "^SEARXNG_PORT=9990$" "$QTGT/.env" 2>/dev/null \
   && grep -q "^FIRECRAWL_PORT=9991$" "$QTGT/.env" 2>/dev/null; then
  echo "[OK] quick-setup used the default ports"
else
  echo "[FAIL] quick-setup did not use the default ports"; PASS=0
fi
if grep -q "^COMPOSE_PROFILES=playwright$" "$QTGT/.env" 2>/dev/null \
   && grep -q "^PLAYWRIGHT_MICROSERVICE_URL=http://playwright-service:3000/scrape$" "$QTGT/.env" 2>/dev/null; then
  echo "[OK] quick-setup defaulted to the Playwright engine"
else
  echo "[FAIL] quick-setup did not default to Playwright"; PASS=0
fi
if grep -q "^FIRECRAWL_API_KEY=" "$QTGT/.env" 2>/dev/null; then
  echo "[FAIL] quick-setup should not have Firecrawl account credentials"; PASS=0
else
  echo "[OK] quick-setup has no Firecrawl account credentials (core mode)"
fi
if [ -d "$QUICKHOME/.agents/skills/local-web-search" ]; then
  echo "[OK] quick-setup installed the local-web-search skill"
else
  echo "[FAIL] quick-setup did not install the local-web-search skill"; PASS=0
fi
...
# answers: use defaults? -> n, target, searxng port, firecrawl port,
#          browserless? -> n, LLM? -> n, account? -> y, key, URL (Enter = default), confirm -> y
printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
  "n" "$TGT3" "" "" "n" "n" "y" "fc-e2e-test-key-123" "" "y" \
...
# answers: use defaults? -> n, target, searxng port, firecrawl port, browserless? -> n (playwright),
#          LLM? -> n, Firecrawl account? -> n (core-only default), confirm -> y
